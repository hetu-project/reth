        ...
        EthApiBuilderProvider<NodeAdapter<T, CB::Components>> + FullEthApiServer + AddDevSigners,
{
    ...
    async fn launch_node(
        ...
        let (consensus_engine_tx, consensus_engine_rx) = unbounded_channel::<BeaconEngineMessage<<T as NodeTypes>::Engine>>();
        ...
        let (beacon_consensus_engine, beacon_engine_handle) = BeaconConsensusEngine::<_,_,_, T>::with_channel(
            client,
            pipeline,
            ctx.blockchain_db().clone(),
            Box::new(ctx.task_executor().clone()),
